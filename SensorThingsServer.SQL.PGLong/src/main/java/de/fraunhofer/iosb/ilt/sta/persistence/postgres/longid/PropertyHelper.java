...
public class PropertyHelper {
    ...
    public static class DatastreamFactory implements PropertyHelper.entityFromTupleFactory<Datastream> {
        ...
        public Datastream create(ResultSet resultSet, Query query, DataSize dataSize) throws SQLException {
            ...
            Long id = resultSet.getLong(qInstance.getMetadata(qInstance.id).getName());
            entity.setId(new LongId(id));
            ...
            ObservedProperty op = observedProperyFromId(resultSet.getLong(qInstance.getMetadata(qInstance.obsPropertyId).getName()));
            ...
            entity.setSensor(sensorFromId(resultSet.getLong(qInstance.getMetadata(qInstance.sensorId).getName())));
            entity.setThing(thingFromId(resultSet.getLong(qInstance.getMetadata(qInstance.thingId).getName())));
        ...
        public Datastream create(Tuple tuple, Query query, DataSize dataSize) {
            ...
            String observedArea = tuple.get(qInstance.observedArea.asText().as(PropertyResolver.OBSERVED_AREA_ALIAS));
            if (observedArea != null) {
                try {
                    Polygon polygon = GeoHelper.parsePolygon(observedArea);
                    entity.setObservedArea(polygon);
    ...
    public static class ObservationFactory implements PropertyHelper.entityFromTupleFactory<Observation> {
        ...
        public Observation create(ResultSet resultSet, Query query, DataSize dataSize) throws SQLException {
            ...
            Long dsId = resultSet.getLong(qInstance.getMetadata(qInstance.datastreamId).getName());
            entity.setDatastream(datastreamFromId(dsId));
            Long mDsId = resultSet.getLong(qInstance.getMetadata(qInstance.multiDatastreamId).getName());
            entity.setMultiDatastream(multiDatastreamFromId(mDsId));

            Long foiId = resultSet.getLong(qInstance.getMetadata(qInstance.featureId).getName());
